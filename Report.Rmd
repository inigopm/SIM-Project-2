...
- DESCRIBIR MODEL 6 Y EXPLICAR PORQUE LO ESCOGEMOS
```{r}
...
- Una vez escogido el mejor model, validamos la calidad del modelo buscando la no-linearidad de la varianza entee lso errores entre las variables predictoras y la clase...
- ANALISIS DEL PEARSON RESIDUAL PLOT(CHATGPT + MARC...)
- MARGINAL MODEL (DISCUTIR SI ESTA OVERFITEANDO)
...
- HEMOS HECHO ANOVA Y STEP PARA QUE NOS ANALICE QUE VARIABLES SON MAS SIGNIFICATIVAS.COMO NOS HA DADO RESULTADOS DISTINTOS, HEMOS CREADO DOS MODELOS DIFERENTES Y COMPARADO CON ANOVA (EN MINUSCILA)
- anova: Pr(>Chi): Es el p-valor asociado con la prueba de Chi-cuadrado. Un p-valor muy bajo (en este caso, menor que 2.2e-16) indica que la mejora en el ajuste del modelo al pasar de mod8 a mod9 es estadísticamente significativa.
```{r}
...
# Resiludal Analysis: Factors

- Explicar lo que hemos visto antes (mensaje de discord)
...
- ANOVA PA VER CUALES SON LAS VARIABLES MAS INFLUYENTES DEL MODELO Y TRANSFORMAR ESAS.
- GREEDY SEARCH PA VER CUAL TIENE MENOS AIC
```{r}
